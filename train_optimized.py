...
@parser.wrap()
def train_optimized(cfg: TrainPipelineConfig):
    ...
    # Compile policy if PyTorch 2.0+ (optional optimization)
    ...
    dataloader = torch.utils.data.DataLoader(
        ...
        prefetch_factor=4 if cfg.num_workers > 0 else None,  # Increased prefetching
    )
    ...
    logging.info("Start optimized offline training")
    ...
    for _ in range(step, cfg.steps):
        ...
        # Optimized data transfer with streams
        if device.type == 'cuda':
    ...
    if eval_env:
        eval_env.close()
    logging.info("End of optimized training")
